  ...
  static analyzeProductMovement(
    ...
    return products.map(product => {
      ...
      const productSales = sales.filter(sale => {
        const saleDate = sale.created_at || sale.createdAt || sale.timestamp;
        return saleDate && new Date(saleDate) >= cutoffDate &&
               sale.items && sale.items.some(item => (item.product_id || item.productId) === product.id);
      });

      const unitsSold = productSales.reduce((total, sale) => {
        const item = sale.items.find(item => (item.product_id || item.productId) === product.id);
